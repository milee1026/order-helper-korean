...
export const FARMERS_ITEMS: ItemConfig[] = [
  {
    ...
    computeTotal: (v) => (Number(v.morningStock) || 0),
  },
  {
  ...
  },
  {
    ...
    computeTotal: (v) => (Number(v.trimmed) || 0) + (Number(v.untrimmed) || 0) / 5 * 3,
  },
  {
...
const REFRIG_ITEMS: ItemConfig[] = [
  ...
  }),
  {
    ...
    fields: [
      { key: 'quarterFull', label: '사용중(1/4 바트)', type: 'number' },
      { key: 'quarterRatio', label: '사용중 비율', type: 'ratio' },
      { key: 'halfStock', label: '손질완료(1/2 바트)', type: 'number' },
      { key: 'halfRatio', label: '손질완료 비율', type: 'ratio' },
